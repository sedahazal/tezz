...
class Model:
    def __init__(
        ...
        self.density = D
        self.layer = layer
        self.layer_heights = append(
            [self.height],
            self.height - cumsum(ones(self.layer) * self.height / self.layer),
        )
        self.K0 = float(K0)
    ...
    def create_part(self):
        ...
        self.model.sketches["__profile__"].Line(
            point1=(self.inf_size, self.height), point2=(self.inf_size, 0)
        )
        self.model.sketches["__profile__"].Line(
            point1=(self.width - self.inf_size, self.height),
            point2=(self.width - self.inf_size, 0),
        )
        self.soilPart.PartitionFaceBySketch(
            faces=self.soilPart.faces[0], sketch=self.model.sketches["__profile__"]
        )
    ...
    def face_partition(self):
        for y in self.layer_heights[1:-1]:
            ...
            self.model.sketches["__profile__"].Line(
                point1=(0, y), point2=(self.width, y)
            )
            self.soilPart.PartitionFaceBySketch(
        ...
        for y in self.layer_heights[1:-1]:
            ...
            self.model.sketches["__profile__"].Line(
                point1=(0, y), point2=(self.inf_size, y)
            )
            self.soilPart.PartitionFaceBySketch(
                faces=face, sketch=self.model.sketches["__profile__"]
            )
        ...
        for y in self.layer_heights[1:-1]:
            ...
            self.model.sketches["__profile__"].Line(
                point1=(self.width - self.inf_size, y), point2=(self.width, y)
            )
            self.soilPart.PartitionFaceBySketch(
                faces=face, sketch=self.model.sketches["__profile__"]
            )
    ...
    def layer_centers(self):
        ...
        for i in range(len(layer_heights)):
            if i == 0:
                ...
                center = (
                    layer_heights[i - 1] + (layer_heights[i] - layer_heights[i - 1]) / 2
                )
    ...
    def create_material(self):
        ...
        for i in range(self.layer):
            ...
            Gmax = (
                3230 * ((2.97 - self.void_ratio) ** 2) * (mean_effective) ** 0.5
            ) / (1 + self.void_ratio)
            E = Gmax * 2.5
            ...
            sand.MohrCoulombPlasticity(
                table=((self.friction_angle, self.dilation_angle),)
            )
            sand.mohrCoulombPlasticity.MohrCoulombHardening(table=((0.1, 0.0),))
            sand.mohrCoulombPlasticity.TensionCutOff(
    ...
    def create_section(self):
        for i in range(self.layer):
            name = "sand{}".format(i)
            face1 = self.soilPart.faces.findAt(
                (self.width / 2, self.height - self.centers[i], 0)
            ).index
            face2 = self.soilPart.faces.findAt(
                (0.1, self.height - self.centers[i], 0)
            ).index
            face3 = self.soilPart.faces.findAt(
                (self.width - 0.1, self.height - self.centers[i], 0)
            ).index
            self.soilPart.Set(
                faces=self.f(self.soilPart.faces, [face1, face2, face3]), name=name
            )
            self.model.HomogeneousSolidSection(name=name + "Section", material=name)
            self.soilPart.SectionAssignment(
                region=self.soilPart.sets[name], sectionName=name + "Section"
            )
    ...
    def edge_sets(self):
        ...
        self.soilPart.Set(
            edges=self.f(
                self.soilPart.edges,
                [left_edge, right_edge, left_edge2, left_edge3, bottom_edge, top_edge],
            ),
            name="Edges",
        )
        ...
        self.soilPart.Set(
            edges=self.f(self.soilPart.edges, [bottom_edge, edge2, edge4]),
            name="HorizontalEdges",
        )
        self.soilPart.Set(
            edges=self.f(self.soilPart.edges, [edge1, edge2, edge3, edge4]),
            name="singleseed",
        )
    ...
    def boundary_conditions(self):
        ...
        self.model.DisplacementBC(
            ...
            name="BC-Y",
            region=sets["HorizontalEdges"],
            u1=0,
            u2=0,
            u3=0,
        )
        self.model.AccelerationBC(
            name="Vibration",
            createStepName="VibrationStep",
            a1=1,
            amplitude="vibration",
            region=self.model.rootAssembly.instances["Soil Part-1"].sets[
                "HorizontalEdges"
            ],
        )
    ...
    def create_step(self):
        ...
        self.model.ImplicitDynamicsStep(
            initialInc=self.time_step,
            timePeriod=self.duration,
            maxInc=self.time_step,
            maxNumInc=1000000,
            name="VibrationStep",
            previous="GravityStep",
        )

    def gravity_load(self):
        self.model.Gravity(
            comp2=-9.81, createStepName="GravityStep", name="Gravity_Load"
        )

    def geostaticstress(self):
        s2 = -self.density * self.height * 9.81
        lc = 0.33
        self.model.GeostaticStress(
            lateralCoeff1=lc,
            name="GS",
            region=self.model.rootAssembly.instances["Soil Part-1"].sets["SoilFace"],
            stressMag1=0,
            stressMag2=s2,
            vCoord1=self.height,
            vCoord2=0,
    ...
    def create_faces(self):
        for i in range(self.layer):
            y = self.centers[i]
            x1 = (0.01, y, 0)
            x2 = (self.width - 0.01, y, 0)
            left_face = self.soilPart.faces.findAt(x1).index
            right_face = self.soilPart.faces.findAt(x2).index
            self.soilPart.Set(
                faces=self.f(self.soilPart.faces, [left_face, right_face]),
                name="infinite_face" + str(i),
            )
    ...
    def generate_mesh(self):
        ...
        self.soilPart.seedEdgeBySize(
            edges=self.soilPart.sets["Edges"].edges,
            size=self.mesh_size,
            constraint=FIXED,
        )

        self.soilPart.seedEdgeByNumber(
            edges=self.soilPart.sets["singleseed"].edges, number=1, constraint=FIXED
        )
    ...
    def create_vibration(self):
        ...
        self.model.TabularAmplitude(
            name="vibration", timeSpan=STEP, smooth=SOLVER_DEFAULT, data=data
        )
    ...
    def change_element_type(self):
        ...
        data = inp.read().replace("CPE4R", "CINPE4")
...
model = Model(
    ...
    92.84519,
    0.5,
    os.path.join(cwd, "self025g-70.txt"),
    1.581,
    4,
    0.388,
    0.64,
